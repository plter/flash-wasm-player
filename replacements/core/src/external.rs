...
impl<'gc> ExternalInterface<'gc> {
    ...
    pub fn get_js_env(&self) -> String {
        for provider in &self.providers {
            return provider.get_js_env();
        }

        let result = json::object! {available:false,msg:"Please set allowScriptAccess: true, example: player.load({url: 'dist/Main.swf',allowScriptAccess: true}"};
        result.to_string()
    }
}
